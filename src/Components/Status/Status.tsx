import "./Status.scss";
import WaterTray from "../../assets/4907845.png";
import coffeeWaste from "../../assets/icons8-tri-des-déchets-50.png";
import coffeeBean from "../../assets/icons8-grains-de-café-50.png";
import coffeeMaking from "../../assets/icons8-chocolat-chaud-50.png";
import { useEffect, useState } from "react";

interface StatusProps {
  isOn: boolean;
  isMakingCoffee: boolean;
  waterLevelState: number;
  waterTrayState: number;
  wasteCoffeeState: number;
  beanFeedState: number;
}

export default function Status(props: StatusProps) {
  const [waterTray, setWaterTray] = useState("N/A");
  const [wasteCoffee, setWasteCoffee] = useState("N/A");
  const [beanFeed, setBeanFeed] = useState("N/A");
  const [coffeeMakingStatus, setCoffeeMakingStatus] = useState("N/A");

  //TODO Handle all level of state and not only 2
  //TODO Maybe there is a better way of doing that ?
  useEffect(() => {
    console.log(props);
    switch (true) {
      case props.waterTrayState <= 1:
        setWaterTray("Full");
        break;
      case props.waterTrayState > 1:
        setWaterTray("Good");
        break;
      default:
        setWaterTray("N/A");
        break;
    }

    switch (true) {
      case props.wasteCoffeeState <= 1:
        setWasteCoffee("Needs Emptying");
        break;
      case props.wasteCoffeeState > 1:
        setWasteCoffee("Good");
        break;
      default:
        setWasteCoffee("N/A");
        break;
    }

    // Switch for BeanFeedState
    switch (true) {
      case props.beanFeedState <= 1:
        setBeanFeed("Low");
        break;
      case props.beanFeedState > 1:
        setBeanFeed("full");
        break;
      default:
        setBeanFeed("N/A");
        break;
    }

    setCoffeeMakingStatus(props.isMakingCoffee ? "Yes" : "No");
  }, [props]);
  return (
    <div className="card">
      <div className="item item--1">
        <img src={WaterTray} alt="Water tray Icon" width={24} height={24}></img>
        <path fill="none" d="M0 0h24v24H0z" />
        <path
          d="M17 15.245v6.872a.5.5 0 0 1-.757.429L12 20l-4.243 2.546a.5.5 0 0 1-.757-.43v-6.87a8 8 0 1 1 10 0zm-8 1.173v3.05l3-1.8 3 1.8v-3.05A7.978 7.978 0 0 1 12 17a7.978 7.978 0 0 1-3-.582zM12 15a6 6 0 1 0 0-12 6 6 0 0 0 0 12z"
          fill="rgba(149,149,255,1)"
        />

        <span className="quantity"> {waterTray}</span>
        <span className="text text--1"> Water tray level </span>
      </div>
      <div className="item item--2">
        <img
          src={coffeeWaste}
          alt="Water tray Icon"
          width={24}
          height={24}
        ></img>
        <path d="M0 0L24 0 24 24 0 24z" fill="none" />
        <path
          fill="rgba(252,161,71,1)"
          d="M16 16c1.657 0 3 1.343 3 3s-1.343 3-3 3-3-1.343-3-3 1.343-3 3-3zM6 12c2.21 0 4 1.79 4 4s-1.79 4-4 4-4-1.79-4-4 1.79-4 4-4zm10 6c-.552 0-1 .448-1 1s.448 1 1 1 1-.448 1-1-.448-1-1-1zM6 14c-1.105 0-2 .895-2 2s.895 2 2 2 2-.895 2-2-.895-2-2-2zm8.5-12C17.538 2 20 4.462 20 7.5S17.538 13 14.5 13 9 10.538 9 7.5 11.462 2 14.5 2zm0 2C12.567 4 11 5.567 11 7.5s1.567 3.5 3.5 3.5S18 9.433 18 7.5 16.433 4 14.5 4z"
        />
        <span className="quantity"> {wasteCoffee} </span>
        <span className="text text--2"> Coffee waste level </span>
      </div>
      <div className="item item--3">
        <img
          src={coffeeBean}
          alt="Water tray Icon"
          width={24}
          height={24}
        ></img>
        <path fill="none" d="M0 0h24v24H0z" />
        <path
          d="M20.083 15.2l1.202.721a.5.5 0 0 1 0 .858l-8.77 5.262a1 1 0 0 1-1.03 0l-8.77-5.262a.5.5 0 0 1 0-.858l1.202-.721L12 20.05l8.083-4.85zm0-4.7l1.202.721a.5.5 0 0 1 0 .858L12 17.65l-9.285-5.571a.5.5 0 0 1 0-.858l1.202-.721L12 15.35l8.083-4.85zm-7.569-9.191l8.771 5.262a.5.5 0 0 1 0 .858L12 13 2.715 7.429a.5.5 0 0 1 0-.858l8.77-5.262a1 1 0 0 1 1.03 0zM12 3.332L5.887 7 12 10.668 18.113 7 12 3.332z"
          fill="rgba(66,193,110,1)"
        />
        <span className="quantity"> {beanFeed} </span>
        <span className="text text--3"> Coffee bean level </span>
      </div>
      <div className="item item--4">
        <img
          src={coffeeMaking}
          alt="Water tray Icon"
          width={24}
          height={24}
        ></img>
        <path fill="none" d="M0 0h24v24H0z" />
        <path
          d="M12 20h8v2h-8C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10a9.956 9.956 0 0 1-2 6h-2.708A8 8 0 1 0 12 20zm0-10a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm-4 4a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm8 0a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm-4 4a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"
          fill="rgba(220,91,183,1)"
        />
        <span className="quantity"> {coffeeMakingStatus} </span>
        <span className="text text--4"> Making coffee ? </span>
      </div>
    </div>
  );
}
